  ...
  async getDropdownData(queryParams) {
    ...
    const specialQueries = {
     ...
     // PRAI_RISK_CLASS_CODE:"SELECT PC_CODE,PC_DESC  FROM PCOM_CODES WHERE PC_TYPE = 'RISK_CLASS' AND PC_VALUE = '20'  AND  (SELECT TRUNC(POL_FM_DT) FROM PGIT_POLICY  WHERE POL_SYS_ID =:pol_sys_id)  BETWEEN PC_EFF_FM_DT AND PC_EFF_TO_DT",
      ...
      CLM_INTM_NO:"SELECT ROWID,TO_CHAR(CI_INTM_NO),NULL FROM PGIT_CLM_INTIMATION WHERE CI_CLM_REGD_YN='0' AND CI_DS_CODE=NVL(:DS_CODE,CI_DS_CODE) AND NVL(CI_CLM_TYPE,'X')=NVL('',NVL(CI_CLM_TYPE,'X')) AND CI_POL_NO=NVL(:POL_NO,CI_POL_NO)",
      ...
      CE_CUST_CODE:"SELECT CUST_CODE,CUST_NAME FROM  PCOM_CUSTOMER",
      CLM_CLOSE_REASON_CODE:"SELECT  PC_CODE,PC_DESC FROM PCOM_CODES WHERE PC_TYPE = 'REAS_CODE'",
      CLM_CAUSE_LOSS:"SELECT PC_CODE, PC_DESC FROM PCOM_CODES WHERE PC_TYPE='CAUSE_LOSS' AND PC_CODE IN (SELECT CCL_CAUSE_LOSS FROM PGIM_CLM_CAUSE_LOSS, PGIM_PRODUCT WHERE CCL_PROD_CODE=PROD_CODE AND PROD_CLASS_CODE=:classCode)",
      CLM_LOSS_CODE:"SELECT PC_CODE,PC_DESC FROM PCOM_CODES WHERE PC_TYPE='NAT_OF_LOSS' AND PC_CODE IN (SELECT LAC_LOSS_CODE FROM PGIM_LOSS_APPL_COVER WHERE LAC_PROD_CODE IN (SELECT PROD_CODE FROM PGIM_PRODUCT WHERE PROD_CLASS_CODE=:classCode))",
    };
    ...
    if (specialQueries[PLD_FIELD_NAME]) {
      ...
      else if(PLD_FIELD_NAME === "CI_POL_NO" || PLD_FIELD_NAME === "CLM_POL_NO")
      {
        ...
        bind.prodCode = prodCode || null;
      } else if(PLD_FIELD_NAME === "CLM_CAUSE_LOSS") {
        bind.classCode = classCode || null;
      } 
      else if(PLD_FIELD_NAME === "CLM_LOSS_CODE") {
        bind.classCode = classCode || null;
